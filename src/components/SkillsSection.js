    ...
    <section id="skills" className="py-20 relative max-w-5xl mx-auto px-6">
      <div className="text-center mb-16">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
          Keahlian & <span className="bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">Tech Stack</span>
        </h2>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2.5">
              {items.map((skill, skillIdx) => (
                <div
                  key={skillIdx}
                  className="group relative flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-900/50 hover:bg-indigo-500/10 border border-white/5 hover:border-indigo-500/30 transition-all duration-300"
                >
                  <span className="text-sm font-medium text-zinc-300 group-hover:text-white">
                    {skill.name}
                  </span>
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-zinc-800 text-zinc-400 group-hover:bg-indigo-500/20 group-hover:text-indigo-300 transition-colors">
                    {skill.level}
                  </span>
                </div>
              ))}
